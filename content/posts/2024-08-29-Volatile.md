...
A use case for `const volatile` is memory-mapped I/O on embedded systems. When hardware is mapped to a specific memory address, the hardware itself writes to that address — no software, no other core. `volatile` prevents the compiler from optimizing away re-reads, and atomics can't help because there's no other *core* to order against. `const` keeps user code from writing to a register it should only read.

```cpp
// Status register on an embedded device
const volatile uint32_t *status_reg = (uint32_t *)0x40021000;

// Hardware updates this independently — must re-read each iteration
